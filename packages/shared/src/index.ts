/**
 * @notecore/shared — tipos de dominio, validaciones y reglas puras.
 *
 * Consumido por `@notecore/api`, `@notecore/web` y `@notecore/mobile`.
 * Antes de crear un tipo o una regla en cualquier cliente, revisa si ya existe aquí.
 */

export * from './types/common.js';
export * from './types/auth.js';
export * from './types/api.js';
export * from './types/schedule.js';
export * from './types/attendance.js';
export * from './types/agenda.js';
export * from './types/calendar.js';
export * from './types/share.js';
export * from './types/semester.js';
export * from './types/social.js';
export * from './types/offline.js';
export * from './types/messaging.js';
export * from './types/updates.js';
export * from './types/panel.js';
export * from './types/moderation.js';
export * from './schemas/common.js';
export * from './schemas/auth.js';
export * from './schemas/schedule.js';
export * from './schemas/attendance.js';
export * from './schemas/agenda.js';
export * from './schemas/calendar.js';
export * from './schemas/share.js';
export * from './schemas/semester.js';
export * from './schemas/social.js';
export * from './schemas/messaging.js';
export * from './schemas/moderation.js';
export * from './logic/attendance.js';
export * from './logic/agenda.js';
export * from './logic/form-errors.js';
export * from './logic/dates.js';
export * from './logic/schedule.js';
export * from './logic/next-class.js';
export * from './logic/widgets.js';
export * from './logic/schedule-import.js';
export * from './logic/calendar.js';
export * from './logic/class-alerts.js';
export * from './logic/tips.js';
export * from './logic/share.js';
export * from './logic/semester.js';
export * from './logic/social.js';
export * from './logic/messaging.js';
export * from './logic/qr.js';
export * from './logic/offline.js';
export * from './logic/sync-engine.js';
export * from './logic/updates.js';
export * from './logic/privacidad.js';
export * from './logic/cuentas-de-prueba.js';
export * from './api/client.js';
export * from './api/auth.js';
export * from './api/schedule.js';
export * from './api/attendance.js';
export * from './api/agenda.js';
export * from './api/calendar.js';
export * from './api/tips.js';
export * from './api/share.js';
export * from './api/semester.js';
export * from './api/social.js';
export * from './api/messaging.js';
export * from './api/live.js';
export * from './api/updates.js';
export * from './api/panel.js';
export * from './api/moderation.js';
export * from './design/tokens.js';
export * from './design/logo.js';
